...
public class AbpGraphQLHttpApiHostModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<AbpGraphQLApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}EasyAbp.Abp.GraphQL.Application.Contracts", Path.DirectorySeparatorChar)));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpGraphQLApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}EasyAbp.Abp.GraphQL.Application", Path.DirectorySeparatorChar)));
            });
        }
        ...
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo {Title = "GraphQL API", Version = "v1"});
        ...
        {
            options.KeyPrefix = "AbpGraphQL:";
        });

        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("GraphQL");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "GraphQL-Protection-Keys");
        }
